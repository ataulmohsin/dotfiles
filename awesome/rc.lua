...
-- {{{ Tags
tags = {
   names = { " WWW", " TERMINAL", " CODE", " MUSIC ", " SOCIAL" },
   layout = { layouts[1], layouts[3], layouts[2], layouts[2], layouts[4] }
}
for s = 1, screen.count() do
...
function ()
    awful.util.spawn_with_shell("mpc prev || ncmpc prev || pms prev || cmus-remote --prev")
    mpdwidget.update()
end)))
next_icon:buttons(awful.util.table.join(awful.button({}, 1,
function ()
    awful.util.spawn_with_shell("mpc next || ncmpc next || pms next || cmus-remote --next")
    mpdwidget.update()
end)))
stop_icon:buttons(awful.util.table.join(awful.button({}, 1,
function ()
    play_pause_icon:set_image(beautiful.play)
    awful.util.spawn_with_shell("mpc stop || ncmpc stop || pms stop || cmus-remote --stop")
    mpdwidget.update()
end)))
play_pause_icon:buttons(awful.util.table.join(awful.button({}, 1,
function ()
    awful.util.spawn_with_shell("mpc toggle || ncmpc toggle || pms toggle || cmus-remote -u")
    mpdwidget.update()
end)))
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ altkey }, "m",
        function ()
            os.execute(string.format("amixer -D pulse set %s toggle", myvolumebar.channel))
            myvolumebar.update()
        end),
